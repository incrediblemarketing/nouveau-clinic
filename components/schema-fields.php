...
if( have_rows('schema_info', 'options') ):
    while ( have_rows('schema_info', 'options') ) : the_row();
		...
		while( have_rows('business_hours') ): the_row();
			...
			$time = '';
			if($sunday): $time .= $sunday; endif;
			if($monday): $time .= ',' . $monday; endif;
			if($tuesday): $time .= ',' . $tuesday; endif;
			if($wednesday): $time .= ',' . $wednesday; endif;
			if($thursday): $time .= ',' . $thursday; endif;
			if($friday): $time .= ',' . $friday; endif;
			if($saturday): $time .= ',' . $saturday; endif;
...
<script type="application/ld+json">
	{ <?php 
		...
		if($business_type): echo '"@type": "'.$business_type.'",';endif;
		if($business_logo): echo '"logo": "'.$business_logo['url'].'",';endif;
		if($business_url): echo '"url": "'.$business_url.'",';endif;
		if($business_price_range): echo '"priceRange": "'.$business_price_range.'",';endif;
		if($business_image): echo '"image": "'.$business_image['url'].'",';endif;
		if($business_same_as): echo '"sameAs": ['.$business_same_as.'],';endif;
		if($business_legal_name): echo '"legalName": "'.$business_legal_name.'",';endif;
		if($business_name): echo '"name": "'.$business_name.'",';endif;
		if($business_alternate_name): echo '"alternateName": "'.$business_alternate_name.'",';endif;
		if($business_phone): echo '"telephone": "'.$business_phone.'",';endif;
		if($business_makes_offer): echo '"makesOffer": ['.$business_makes_offer.'],';endif;
		if($time): echo '"openingHoursSpecification": ['.$time.'],';endif;
		if($business_founder_name):
			echo '"founder":{';
				echo '"@type": "Person",';
				if($business_founder_name): echo '"name": "'.$business_founder_name.'",';endif;
				if($business_founder_prefix): echo '"honorificPrefix": "'.$business_founder_prefix.'",';endif;
				if($business_founder_given_name): echo '"givenName": "'.$business_founder_given_name.'",';endif;
				if($business_founder_family_name): echo '"familyName": "'.$business_founder_family_name.'",';endif;
				if($business_founder_job_title): echo '"jobTitle": "'.$business_founder_job_title.'",';endif;
				if($business_founder_image): echo '"image": "'.$business_founder_image['url'].'",';endif;
				if($business_founder_works_for): echo '"worksFor": "'.$business_founder_works_for.'",';endif;
				if($business_founder_affiliation): echo '"affiliation": "'.$business_founder_affiliation.'",';endif;
				if($business_founder_member_of): echo '"memberOf": "'.$business_founder_member_of.'",';endif;
				if($founder_same_as): echo '"sameAs": ['.$founder_same_as.']';endif;
			echo '},';
		endif;
		if($address_info):
			echo '"address":{';
				echo '"@type": "PostalAddress",';
				if($address_locality): echo '"addressLocality": "'.$address_locality.'",';endif;
				if($address_region): echo '"addressRegion": "'.$address_region.'",';endif;
				if($street_address): echo '"streetAddress": "'.$street_address.'",';endif;
				if($postal_code): echo '"postalCode": "'.$postal_code.'"';endif;
			echo '},';
		endif;
		if($lat || $lng):
			echo '"geo":{';
				echo '"@type": "GeoCoordinates",';
				if($lat): echo '"latitude": "'.$lat.'",';endif;
				if($lng): echo '"longitude": "'.$lng.'"';endif;
			echo '},';
		endif;
		if($contact_type):
			echo '"contactPoint":{';
				echo '"@type": "contactPoint",';
				if($contact_type): echo '"contactType": "'.$contact_type.'",';endif;
				if($contact_phone): echo '"telephone": "'.$contact_phone.'",';endif;
				if($contact_url): echo '"url": "'.$contact_url.'"';endif;
			echo '}';
		endif ?>
	}
</script>